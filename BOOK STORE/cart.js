  ...
  if (cartBody) {
    if (localStorage.getItem("cartData") != null) {
      ...
      for (var i = 0; i < myCartData.length; i++) {
        ...
        var subtotal =
          Number(itemData[2].replace("₹", "").trim()) * Number(itemData[3]);
        ...
        cartRow
          .getElementsByClassName("fa-times-circle")[0]
...
function updateSubtotal(event) {
  ...
  curCartRow.getElementsByClassName("cart-item-subtotal")[0].innerText =
    "₹ " + subtotal;
...
function updateCartTotal() {
  ...
  for (var i = 0; i < cartRows.length; i++) {
    ...
    var subtotalElement =
      cartRow.getElementsByClassName("cart-item-subtotal")[0];

    var subtotal = parseFloat(
      subtotalElement.innerText.replace("₹", "").trim()
    );
